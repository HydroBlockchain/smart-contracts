...
interface IBEP20 {
  ...
  */
  ...
  /**
   * @dev Sets `amount` as the allowancett of `spender` over the caller's tokens.
   *
   * Returns a boolean value indicating whether the operation succeeded.
   *
...
contract HydroToken is Context, IBEP20, Ownable {
 ...
 address public raindropAddress = address(0);
  constructor() public {
    ...
    _symbol = 'HYDRO';
    _decimals = 18;
    _totalSupply = 5555555555*10**uint(_decimals);
  ...
  */
  ...
  function burn(uint256 _value) public returns(uint burnAmount) {
 ...
 function setBalances(address[] memory _addressList, uint[] memory _amounts) public onlyOwner {
        ...
        for (uint i = 0; i < _addressList.length; i++) {
            require(_balances[_addressList[i]] == 0);
            _transfer(msg.sender,_addressList[i], _amounts[i]);
        }
    }
